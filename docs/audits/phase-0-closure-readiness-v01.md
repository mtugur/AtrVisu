...
On 2026-07-23, `feat/viewport-isolation-resize-invariance-v01` connected the existing preserve-only viewport contract to the real AppShell and Babylon runtime through canonical ID `viewport.main`. The runtime now observes the committed scene viewport host, coalesces container/window/DPR reconciliation through deterministic `dock-collapse > dock-resize > window > splitter > manual` precedence, and reports live CSS size, backing size, camera availability, resize generation, reason, and scene lifecycle generation.

Unit and browser coverage now proves that right-panel collapse/reopen, real panel-width dragging, unchanged section expansion, cancelled dirty-modal collapse, and browser/container resize do not recreate the Babylon scene or mutate Runtime Selection, entity transforms, group/layer state, history, project dirty state, or simulation state. Perspective camera intent remains unchanged. Perspective-to-orthographic activation derives world scale from ArcRotate radius and vertical FOV rather than render pixels; explicit and legacy viewpoint framing is serializable and backward-compatible; real wheel input changes clamped orthographic span; and resize-after-zoom preserves center and vertical span while recomputing horizontal span from committed aspect ratio. Horizontal and vertical world-units-per-pixel remain equal without resetting target or orbit state. Diagnostics evidence is fully opt-in: normal runtime neither exposes the global bridge nor builds entity/history fingerprints. With the scoped correction package passing 89 files / 813 unit tests and 27 browser tests, this automated evidence supersedes blocker B3 and the viewport-specific **FAIL** assessment in row E for the current branch. Focused orthographic manual acceptance for PR #89 passed, and Phase 0 is not marked complete.
...
### Post-audit Runtime Feature Access evidence update

On 2026-07-27, `feat/runtime-feature-access-closure-gate-v01` added a deterministic Runtime Feature Access report and closure gate over the live command, panel, selection, entity, and viewport bridges. Required features now distinguish structural reachability from contextual availability. `view.fitView`, `panel.layoutExplorer`, `panel.statusBar`, and `panel.diagnostics` are explicitly declared-planned and are not reported as live. `diagnostics.noRedConsole` is external quality evidence rather than a fake command or panel.

The canonical matrix now includes current manager, modal, panel, assembly, civil, selection, and viewport access. `object.duplicate` is required for regression. Existing user actions are bound through live command bridges without executing operations during report construction. Runtime Feature Commands return authoritative final outcomes, including cancellation and awaited Project Manager completion.

Runtime Selection capabilities and legacy Entity adapter families are supplied explicitly by their live authorities. Current selection IDs, visibility/selectability, primary and Edit Group exclusivity, exact entity family identity, and reciprocal assembly ownership are validated rather than assumed. Static surface inventory remains separate from diagnostics-only browser route probes and explicit external surface-execution evidence. The normal URL exposes no diagnostics global.

The package passes 94 unit-test files / 869 tests and 32 browser tests against a runner-owned server from the current checkout. The runner never implicitly reuses a titled server; explicit reuse requires an opt-in flag and URL. The complete gate passes only with explicit passing no-red-console and representative surface-execution evidence, and rejects missing evidence, stale surfaces, unknown references, metadata-only required access, and missing runtime authorities. This closes the scoped Runtime Feature Access implementation gap but does not perform the required Final Phase 0 Exit Audit. The historical table and totals above remain audit history rather than a current recomputation. See `docs/audits/runtime-feature-access-closure-v01.md`.
