...
Unit and browser coverage now proves that right-panel collapse/reopen, real panel-width dragging, unchanged section expansion, cancelled dirty-modal collapse, and browser/container resize do not recreate the Babylon scene or mutate Runtime Selection, entity transforms, group/layer state, history, project dirty state, or simulation state. Perspective camera intent remains unchanged. Orthographic framing resolves both explicit and Babylon default/null bounds, preserves center and vertical world span, recomputes horizontal span from committed aspect ratio, and maintains equal horizontal/vertical world-units-per-pixel without resetting target or orbit state. Diagnostics evidence is fully opt-in: normal runtime neither exposes the global bridge nor builds entity/history fingerprints. With the scoped correction package passing 89 files / 795 unit tests and 26 browser tests, this evidence supersedes blocker B3 and the viewport-specific **FAIL** assessment in row E for the current branch.
