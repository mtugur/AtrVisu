...
- Direct Plan drag prioritizes perspective-correct grab-point lock and continuous bounded Plan movement; it does not force an artificial universal screen-vertical-to-camera-direction mapping. Pointer-down captures the real finite Babylon picked point and freezes its world Y. Well-conditioned pointer rays intersect that same horizontal plane; near-horizon or otherwise ill-conditioned frames continue from the previous valid delta through a damped, increment-bounded local screen Jacobian. Only Plan X/Y changes, every domain Elevation value remains unchanged and rigid Group members receive one identical delta from the picked member surface.
...
- Correction E unit coverage verifies real picked-point capture, forward plane intersections from above and below, tight perspective and orthographic grab lock, shallow/level/near-horizon bounded Jacobian continuation, singular-neighborhood exit continuity and the Standard Machine/imported GLB/Civil/Group elevation matrix across camera alpha, zoom, target Y 0/25 metres and steep/top-down framing.
- Correction E Chromium coverage performs multi-step real pointer drags and checks every representative diagnostic frame. A moderate-height machine, 20 and 50 metre columns, a 25 metre standard machine, a 25 metre imported GLB machine and a mixed-elevation rigid Group are exercised from valid camera-above/camera-below views and low/high member surfaces. Well-conditioned frames enforce at most two CSS pixels of pointer-to-anchor error; the explicit near-horizon case enforces finite bounded continuation without a second click or teleport. Elevation, one-step Undo, rigid member deltas, Babylon lifecycle and no-red-console/page-error guarantees remain protected.
...
- Evidence artifact: `pf3a-global-iconography-density` with twenty-nine named exact-head runtime captures. The accepted twenty-five remain; `26-machine-grab-lock-camera-above.png`, `27-machine-grab-lock-camera-below.png`, `28-civil-grab-lock-elevated.png` and `29-group-grab-lock.png` cover Correction E.
