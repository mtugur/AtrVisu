...
- The remaining React feedback edge was the Inspector Auto effect treating each recreated `RuntimeSelection` object as a new semantic selection. That identity dependency could repeat the open/collapse preference write path during selection reconciliation and external-store publication. The effect now compares the canonical ordered selection-ID signature and reacts only when selection meaning or Auto/Pinned mode changes. Existing same-value preference and panel-reachability equality guards remain in place. A single long-lived Chromium session migrates legacy preferences, exercises current persisted preferences, Inspector Auto/Pinned/open/collapse, Primary Dock open/collapse, Machine and Group selection, floor clear, Groups Edit/Done, Arrange, manipulator drag and hard reload under the maximum-depth/console/page-error guard.
