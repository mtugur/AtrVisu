...
- Floor Area is physical slab/world geometry. Floor Area, Machine, Wall, Column and Beam render in the same physical-world rendering group so normal scene depth determines their visibility. Walkway, Restricted Area, Reference Zone and other non-physical planning/reference Civil types remain in the later planning/reference group, whose existing depth-preservation policy remains active.
...
- Floor Area Inspector labels are `Level`, `Top Elevation above Level (mm)`, read-only `Top Surface World Elevation (mm)`, and read-only `Bottom Surface World Elevation (mm)`. The bottom readout projects canonical `CivilReferenceItem.positionMm.zMm`; it is not editable and does not create a second elevation authority.
...
No Level-as-Library-asset, Level-as-Civil-item, Floor-Area-to-Level inference, Machine support-height inference, overlap/contact solver, automatic Floor Area Level creation, Z-position inference, Layer reuse, second relative-elevation store, visual-only offset, always-on-top geometry, forced Floor opacity, transparent-Beam depth-write workaround, partial movement of locked content, silent reassignment on deletion, schema fork, BIM hosting, story visibility, clipping, basement Levels, movement fallback or PF-3B redesign.

## Acceptance

Legacy layouts retain exact world elevations and normalize to Ground. New Machine/Build placement uses Active Level plus existing local/default elevation, except Floor Area whose top surface defaults to the Active Level datum. Ground `0 mm` with a `350 mm` Floor Area yields Inspector bottom `-350 mm`, top `0 mm`, and Machine base `0 mm`; Level 2 `6000 mm` yields Inspector bottom `5650 mm`, top `6000 mm`, and Machine base `6000 mm`. Machine/non-Floor Civil relative elevation and Floor Area top-relative elevation remain derived from canonical world geometry. Reassignment, datum change, and Civil type transitions preserve the applicable relative anchor. A Floor Area at top-relative `0 mm` and height `350 mm` transitions to a Wall at base-relative `0 mm`, and the reverse transition restores the Floor Area bottom to `-350 mm` on Ground. Floor Area thickness edits preserve its top surface. Machine and Beam visibly participate in physical depth when their footprints overlap an opaque Floor Area from above; the same opaque slab may occlude geometry behind it from below, while transparent Floor Area blending remains intact. Datum change is one atomic Undo/Redo transaction and rejects any locked assigned content. Assigned Level deletion is blocked. Save/reload/export/import preserve Levels, assignments, signed Floor Area bottoms and canonical world geometry. A 25,000 mm Level remains finite and console-clean without changing ADR-001's accepted limitation.
