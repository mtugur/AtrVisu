...
- The old CivilReferencePanel add-button stack is not a normal creation surface. Insert-menu per-type Build add shortcuts are removed from the normal menu. The six registered per-type Civil command IDs remain runtime-bound for compatibility but are declaratively excluded from Command Palette discovery; `civil.addPrimitive` through Library Build is the single normal creation route.
