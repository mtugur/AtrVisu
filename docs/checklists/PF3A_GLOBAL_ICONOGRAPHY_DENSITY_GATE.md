...
| Runtime safety | Inspector Auto compares the canonical ordered selection-ID signature rather than render-volatile Runtime Selection identity; same-value UI-preference writes emit no snapshot; semantically unchanged Runtime Panel reachability preserves its state reference; one long-lived migrated/current preference session exercises Inspector Auto/Pin/open/collapse, Primary Dock open/collapse, selection, Groups Edit/Done, Arrange, manipulator drag and reload under the maximum-update-depth/console/page-error guard |
