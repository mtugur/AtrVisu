...
| Perspective-correct direct Plan drag | Direct Plan drag prioritizes perspective-correct grab-point lock and continuous bounded Plan movement; it does not force an artificial universal screen-vertical-to-camera-direction mapping. Pointer-down captures a real finite picked surface, normal frames intersect its fixed horizontal plane, near-horizon frames use bounded Jacobian continuation, and Machine/Civil/Group Elevation remains unchanged |
...
| Visual evidence | `pf3a-global-iconography-density` contains twenty-nine exact-head captures, retaining the accepted twenty-five and adding `26-machine-grab-lock-camera-above.png`, `27-machine-grab-lock-camera-below.png`, `28-civil-grab-lock-elevated.png` and `29-group-grab-lock.png` |
