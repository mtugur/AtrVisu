...
| Continuous body drag | Machine, Civil and Group Plan Move distinguish `applied`, valid snapped `noop` and `blocked`; no-op writes no state/history and keeps the same gesture active until later movement applies or pointer-up ends it |
| Fixed picked-elevation plane | Direct Machine/Civil body pointer-down captures the actual finite picked-point elevation; every later frame intersects only that one floor-parallel plane. Immutable canonical starts produce one Plan delta, rigid Group/multi members share it, Elevation remains invariant and the first applied frame records the gesture's sole Undo transaction |
| Accepted limitation | Approximately 20 m near/below-plane camera geometry may not yield a usable forward intersection. Do not introduce a second solver, adaptive mapping, smoothing, hidden fallback or PositionGizmo; keep values finite and console clean |
| Interaction governance | Interaction Standard §3 and ADR-001 explicitly authorize the Phase-1 direct-body-drag deviation from `docs/benchmarks/INTERACTION_BENCHMARK_BASELINE_V1.md`; Product Constitution and merged governance checks remain unchanged |
...
| Visual evidence | `pf3a-global-iconography-density` retains accepted iconography captures and includes `33-console-clean-persisted-state.png`, `34-body-drag-machine.png`, `35-body-drag-civil.png`, `36-body-drag-group.png`, `37-body-drag-imported-glb.png` and `38-console-clean-migrated-current-session.png` |
