...
| Camera-azimuth plan drag | Pointer-down captures one screen-relative Plan basis and framing scale for the complete Machine/Civil/Group gesture. Screen right/left remain camera-relative Plan right/left and screen down/up remain toward/away from the camera across pitch crossings. Perspective and orthographic sensitivity depend on camera framing, never object or picked-point height; pointer drag remains Plan-only and preserves every domain Elevation value |
...
| Visual evidence | `pf3a-global-iconography-density` contains twenty-five exact-head captures, retaining the accepted twenty-two and adding `23-plan-drag-camera-above.png`, `24-plan-drag-camera-below.png` and `25-plan-drag-elevated-group.png` |
