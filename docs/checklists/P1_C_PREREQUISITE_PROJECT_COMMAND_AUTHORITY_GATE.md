...
| Save available without modal | Chromium diagnostic route creates a persisted revision after Project Manager closes | PASS | Yes | E2E `project save and active export execute while Project Manager is closed` |
...
| Import execution available without modal | Live binding accepts `{ file: File }` independently of modal mounting | PASS | Yes | Authority unit tests; persistent-provider E2E |
...
| Persistent import provider proven | Exactly one hidden App input remains mounted before, during, and after modal use | PASS | Yes | Static architecture test; persistent-provider E2E |
...
| Selection/history/viewport invariants preserved | Import E2E compares selection, undo/redo, dirty state, lifecycle generation, and canvas count | PASS | Yes | Persistent-provider E2E |
| Audit/build/unit/E2E passed | 0 vulnerabilities; build passed; 105/1011 unit; 37/37 E2E | PASS | Yes | Audit report section 11 |
