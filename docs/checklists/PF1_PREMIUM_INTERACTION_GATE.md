...
| Professional engineering strip | PASS | History/Selection/Display/Precision/Arrange groups, shared group chrome, visible concise desktop labels, accessible names/tooltips, and truthful pressed state |
...
| Viewpoints and Selection Tools toggles | PASS | Open, close, contribution switch, pressed-state truth, and persisted-height reopen use Bottom Dock/Panel/UI Preference authorities |
...
| Selection Tools utility | PASS | Compact 0/1 context state, common 2+ operations, truthful 3-object distribution disablement, collapsed pair Advanced, and connection snap only for eligible exact-two-machine context |
| Keyboard Nudge placement | PASS | Nudge settings use the existing `NudgeSettings` state in Precision Placement / Measurement Helpers and no longer appear in Selection Tools |
...
| Focused validation | PASS | PF-1C component: 8 files / 62 tests; focused Chromium covers contextual Measurement Helpers, exact-two connection disclosure, delete naming, compact overflow keyboard behavior, feature observation, and live command surfaces |
...
| Final manual visual acceptance | REQUIRED | Integrated startup, grouped chrome, property Inspector, Selection Tools, Viewpoints toggle, labels, Help, and responsive views require user acceptance |
