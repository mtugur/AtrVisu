...
fit-to-page top plan, orientation, origin, and overall extents. Page 2 consumes
the same snapshot's P1-E `report` mappings for the equipment schedule.
...
document. It is MIT licensed and client-side. Both serializers are loaded on
demand, are isolated from the domain model, and introduce no storage schema or
remote execution path. The package lock remains npm-owned and the final audit
...
- Serializer tests inspect the XLSX OpenXML package and parse the PDF metadata,
  page count, dimensions, and canonical plan model.
...
- A bounded final manual acceptance reviews the workbook, both PDF pages, and
  presentation image readability from a genuine ATARA line.
