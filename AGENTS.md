# AGENTS.md — AtrVisu Agent Operating Rules v0.6
...
4. Kullanıcı etkileşimi değişiyorsa `docs/standards/ATRVISU_BENCHMARK_EVIDENCE_STANDARD.md`
5. Kullanıcı etkileşimi değişiyorsa `docs/standards/ATRVISU_INTERACTION_STANDARD.md`
6. Kullanıcı etkileşimi uygulanacaksa `docs/protocols/INTERACTION_DELIVERY_PROTOCOL.md`
7. İlgili diğer dosyalar: `docs/standards/*`
8. İlgili normatif ürün dosyaları: `docs/product/*`
9. `docs/protocols/CODEX_SYNC_PROTOCOL.md`
10. İlgili `docs/checklists/*`
11. Gerekirse ilgili ADR dosyası
...
- User-facing interaction için normatif sözleşme yoksa implementasyon yapılmaz. Önce task-similar benchmark evidence + interaction contract; gerekiyorsa ADR; sonra kod.
- Benchmark iddiası ürün adıyla veya hafızayla kabul edilmez; `ATRVISU_BENCHMARK_EVIDENCE_STANDARD.md` uyarınca traceable official/credible evidence gerekir.
- Benchmark araştırması implementasyondan sonra mevcut kodu gerekçelendirmek için geriye dönük seçilemez.
...
Bir interaction implementation paketi normalde bir implementasyon turu + aynı frozen contract içinde en fazla bir consolidated correction batch kullanır. Bunun ötesi `INTERACTION_DELIVERY_PROTOCOL.md` gereği contract/benchmark review’a döner.

## Geliştirme Disiplini
- Contract-first.
- Benchmark-first for user-facing interaction.
- Project-to-repository contract sync before implementation.
- Benchmark evidence before interaction implementation.
- Küçük branch/slice.
...
- The normal delivery loop is: sync/freeze source + benchmark evidence + contract, implement, run CI, perform one comprehensive review, apply at most one consolidated correction batch when the frozen contract allows it, rerun CI, complete one final manual acceptance when visible behavior changed, then merge.
- Routine Git, test, CI, benchmark verification and reproducible runtime investigation are agent-owned. Ask the user only for genuine final manual visual/runtime acceptance or destructive/ambiguous decisions; do not use the user as an exploratory QA loop.
...
- Any PR changing interaction must complete `docs/checklists/INTERACTION_CHANGE_GATE.md` and point to the applicable traceable benchmark evidence record.
- An implementation agent may not edit Product Constitution / Interaction Standard / Benchmark Standard merely to bless code already written. Contract change and implementation must follow `INTERACTION_DELIVERY_PROTOCOL.md` ordering.
