...
const captureP1Bld2Screenshot = async (page: Page, fileName: string) => {
  if (!captureP1Bld2Evidence) return;
  await mkdir(p1Bld2EvidenceDirectory, { recursive: true });
  await page.screenshot({ path: join(p1Bld2EvidenceDirectory, fileName), fullPage: false });
};
...
const waitForSceneRenderFrames = (page: Page) => page.evaluate(() => new Promise<void>((resolve) => {
...
type SceneMachineLabel = { instanceId: string; meshName: string; text: string | null; visible: boolean };
...
const expectOneSceneLabelPerMachine = async (page: Page, expectedTexts?: readonly string[]) => {
  await expect.poll(async () => {
    const labels = await readMachineSceneLabels(page);
    return {
      count: labels.length,
      uniqueInstances: new Set(labels.map((label) => label.instanceId)).size,
      texts: labels.map((label) => label.text).sort()
    };
  }).toEqual({
    count: expectedTexts?.length ?? (await getMachineIds(page)).length,
    uniqueInstances: expectedTexts?.length ?? (await getMachineIds(page)).length,
    texts: expectedTexts ? [...expectedTexts].sort() : (await readMachineSceneLabels(page)).map((label) => label.text).sort()
  });
};
...
test("runtime feature access complete gate is bound to observed visible command routes", async ({ page }) => {
  ...
  const snapMachineIds = await getMachineIds(page);
  await clickSceneMachine(page, snapMachineIds[3]);
  if ((await propertiesSection.getAttribute("aria-expanded")) !== "true") {
    await propertiesSection.click();
  }
  ...
  await clickSceneMachine(page, snapMachineIds[4]);
...
test("PF-1 Explorer F2 rename preserves canonical identity and history", async ({ page }) => {
  ...
  const canvas = page.getByLabel("AtrVisu 3D workspace");
  const lifecycleGeneration = await canvas.getAttribute("data-scene-lifecycle-generation");
  await expectOneSceneLabelPerMachine(page, ["Flow Pack Machine"]);
  ...
  await page.keyboard.press("F2");
  const cancelledRename = explorer.getByRole("textbox", { name: "Rename Flow Pack Machine - Line 2" });
  await cancelledRename.fill("Transient Name");
  await cancelledRename.press("Escape");
  await expect(cancelledRename).toHaveCount(0);
  await expectOneSceneLabelPerMachine(page, ["Flow Pack Machine - Line 2"]);
  ...
  await getCommandBarCommand(page, "view.toggleLabels").click();
  await expect.poll(async () => (await readMachineSceneLabels(page))[0]?.visible).toBe(true);
  ...
  await expect.poll(() => page.evaluate(() => Boolean(
    window.localStorage.getItem("atrvisu.autosavedLayout.v1")
  ))).toBe(true);
  ...
  await expectOneSceneLabelPerMachine(
    recoveryPage,
    ["Flow Pack Machine - Line 2", "Flow Pack Machine - Line 2"]
  );
...
test("PF-1C Connection Point Snap stays responsive for two eligible machines", async ({ page }) => {
  ...
  await expect(snapPanel).toHaveAttribute("data-snap-mode", "product-flow");
...
test("workbench chrome keyboard and responsive geometry preserve the editor lifecycle", async ({ page }) => {
  ...
  const assertGeometry = async (width: number, height: number) => {
    ...
    await expect.poll(async () => (await getRuntimeViewportSnapshot(page)).viewport?.cssWidth ?? 0)
      .toBeGreaterThan(0);
    const geometry = await page.evaluate(() => {
      const rect = (selector: string, optional = false) => {
        const element = document.querySelector(selector);
        if (!element) {
      ...
      return {
        ...
        panel: rect('[data-testid="right-panel"]', true),
    ...
    if (width <= 1100) {
      expect(geometry.panel).toBeNull();
      await expect(page.getByRole("button", { name: "Expand Inspector" })).toBeVisible();
    } else {
      expect(geometry.panel).not.toBeNull();
      expect(geometry.command.bottom).toBeLessThanOrEqual((geometry.panel?.top ?? 0) + 1);
    }
...
test("Connection Point Snap uses the authoritative exact-two-machine context", async ({ page }) => {
  ...
  await page.getByTestId("viewport-arrange-bar").getByRole("button", { name: "Connect & Snap" }).click();
  await expect(page.getByTestId("connect-and-snap-popover")).toBeVisible();
  await expect(page.getByTestId("connection-point-snap-panel")).toHaveAttribute("data-snap-mode", "product-flow");
...
test("rigid assembly projection renders one group arrange entity without exposing members", async ({ page }) => {
  ...
  const distributeHorizontal = selectionTools.getByRole("button", { name: "Horizontal", exact: true });
  const distributeVertical = selectionTools.getByRole("button", { name: "Vertical", exact: true });
  const equalGapX = selectionTools.getByRole("button", { name: "Equal Gap X" });
  const equalGapY = selectionTools.getByRole("button", { name: "Equal Gap Y" });
...
test("scene lifecycle stays stable through selection and accepted pointer drag", async ({ page }) => {
  ...
  await dragSceneMachine(page, machineId, 90, 45);
  await expect.poll(async () => (
    await readCanvasRecord<PlanPosition>(page, "data-machine-plan-positions")
  )[machineId]).not.toEqual(before);
  await expect(canvas).toHaveAttribute(
    "data-scene-lifecycle-generation",
    initialLifecycleGeneration ?? ""
  );
...
test("high-grab drag preserves machine civil and rigid-group elevations", async ({ page }) => {
  ...
  await capturePf3aScreenshot(page, "34-body-drag-machine.png");
  ...
  await capturePf3aScreenshot(page, "35-body-drag-civil.png");
  ...
  await capturePf3aScreenshot(page, "36-body-drag-group.png");
  await capturePf3aScreenshot(page, "19-group-drag-high-grab.png");
  expect(errors).toEqual([]);
});

test("imported GLB body drag preserves elevation and scene lifecycle", async ({ page }) => {
  const errors = await startNativeAssetTest(page);
  const canvas = page.getByLabel("AtrVisu 3D workspace");
  await openProjects(page);
  await page.getByTestId("new-project-name").fill("Body Drag Imported Asset");
  await page.getByTestId("new-customer-name").fill("E2E Customer");
  await page.getByTestId("create-project").click();
  await page.getByTestId("close-project-manager").click();
  const importDialog = await prepareNativeAsset(page, { exerciseCalibration: false });
  ...
  const before = (await readCanvasRecord<PlanPosition>(page, "data-machine-plan-positions"))[machineId];
  await dragSceneMachine(page, machineId, 55, 24);
  await expect.poll(async () => (
    await readCanvasRecord<PlanPosition>(page, "data-machine-plan-positions")
  )[machineId]).not.toEqual(before);
  expect(await readCanvasRecord<number>(page, "data-machine-elevations-mm")).toEqual(elevationBefore);
  await expect(canvas).toHaveAttribute("data-scene-lifecycle-generation", lifecycleGeneration ?? "");
  await expect(canvas).not.toHaveAttribute("data-plan-move-manipulator");
  await capturePf3aScreenshot(page, "37-body-drag-imported-glb.png");
  expect(errors).toEqual([]);
});

test("near-plane 20 metre body drag stays finite without hidden remapping", async ({ page }) => {
  const errors = collectPageErrors(page);
  await openCleanApp(page);
  await addCanonicalAtaraMachine(page, "Robot Palletizer", ["Palletizing", "Robot Palletizers"]);
  await waitForMachineDiagnostics(page, 1);
  ...
  await dragSceneMachine(page, machineId, 28, 0);
  const after = (await readCanvasRecord<PlanPosition>(page, "data-machine-plan-positions"))[machineId];
  expect(Number.isFinite(after.xMm)).toBe(true);
  expect(Number.isFinite(after.yMm)).toBe(true);
  expect(await readCanvasRecord<number>(page, "data-machine-elevations-mm")).toEqual(elevationBefore);
  await expect(canvas).not.toHaveAttribute("data-plan-move-manipulator");
  expect(errors).toEqual([]);
});
...
test("locked Machine Layer rejects Level datum atomically through the real panel workflow", async ({ page }) => {
  const errors = collectPageErrors(page);
  await openCleanApp(page);

  await openPrimaryDockPanel(page, "panel.levels");
  const levelPrompts = ["Level 2", "6000"];
  const answerLevelPrompts = async (dialog: Dialog) => dialog.accept(levelPrompts.shift() ?? "");
  page.on("dialog", answerLevelPrompts);
  await page.getByTestId("add-level").click();
  page.off("dialog", answerLevelPrompts);
  const level2 = page.getByTestId(/level-row-level-/).filter({ hasText: "Level 2" });
  await level2.locator(".layer-main-button").click();
  ...
  const floorProperties = page.getByTestId("civil-reference-properties");
  await expect(floorProperties.getByLabel("Civil Top Elevation above Level")).toHaveValue("0");
  await expect(floorProperties.getByTestId("civil-world-elevation")).toContainText("6000 mm");

  await openPrimaryDockPanel(page, "panel.layoutExplorer");
  await page.getByRole("button", { name: /Flow Pack Machine/ }).click();
  await openPrimaryDockPanel(page, "panel.layers");
  page.once("dialog", (dialog) => dialog.accept("Locked Machine Layer"));
  await page.getByTestId("add-layer").click();
  await machineProperties.getByLabel("Layer").selectOption({ label: "Locked Machine Layer" });
  const lockedLayer = page.locator(".layer-row").filter({ hasText: "Locked Machine Layer" });
  await lockedLayer.getByRole("button", { name: "Lock Locked Machine Layer", exact: true }).click();
  await expect(machineProperties.getByLabel("Layer")).toBeDisabled();

  const before = await getRuntimeViewportSnapshot(page);
  const canvas = page.getByLabel("AtrVisu 3D workspace");
  const machineElevationsBefore = await readCanvasRecord<number>(page, "data-machine-elevations-mm");
  const civilElevationsBefore = await readCanvasRecord<number>(page, "data-civil-elevations-mm");
  let rejectionMessage = "";
  const answerDatumAndRejection = async (dialog: Dialog) => {
    if (dialog.type() === "prompt") {
      await dialog.accept("6500");
      return;
    }
    rejectionMessage = dialog.message();
    await dialog.accept();
  };
  page.on("dialog", answerDatumAndRejection);
  await openPrimaryDockPanel(page, "panel.levels");
  await level2.getByRole("button", { name: "Set Level 2 datum" }).click();
  await expect.poll(() => rejectionMessage).toContain("assigned entity is locked");
  page.off("dialog", answerDatumAndRejection);

  await expect(level2).toContainText("6000 mm datum");
  await expect(canvas).toHaveAttribute("data-machine-elevations-mm", JSON.stringify(machineElevationsBefore));
  await expect(canvas).toHaveAttribute("data-civil-elevations-mm", JSON.stringify(civilElevationsBefore));
  const after = await getRuntimeViewportSnapshot(page);
  expect(after.invariants.undoDepth).toBe(before.invariants.undoDepth);
  expect(after.invariants.undoStack).toEqual(before.invariants.undoStack);
  expect(errors).toEqual([]);
});

test("Floor Area uses the Level FFL top anchor and preserves physical world depth", async ({ page }) => {
  const errors = collectPageErrors(page);
  await openCleanApp(page);
  const canvas = page.getByLabel("AtrVisu 3D workspace");
  ...
  await expect.poll(async () => Object.keys(
    await readCanvasRecord<number>(page, "data-civil-elevations-mm")
  ).length).toBe(1);
  ...
  expect(await applyRuntimeViewportCameraState(page, {
    mode: "perspective", alpha: Math.PI / 4, beta: 2.7, radius: 34,
    targetX: -2, targetY: 0, targetZ: -2
  })).toBe(true);
  await waitForSceneRenderFrames(page);
  await getMachineScreenBounds(page, machineId);
  ...
  await openPrimaryDockPanel(page, "panel.layoutExplorer");
  await page.locator(`[data-entity-id="civil:${floorId}"]`).click();
  await properties.getByLabel("Civil Opacity").fill("0.35");
  await expect.poll(async () =>
    (await readCanvasRecord<{ color: string; opacity: number }>(page, "data-civil-rendered-styles"))[floorId]?.opacity
  ).toBeCloseTo(0.35);
  expect(await applyRuntimeViewportCameraState(page, {
  ...
  await expect.poll(async () => Boolean((await readCanvasRecord<ScreenPoint>(page, "data-civil-screen-points"))[floorId])).toBe(true);
  const dragPoint = (await readCanvasRecord<ScreenPoint>(page, "data-civil-screen-points"))[floorId];
  const canvasBox = await canvas.boundingBox();
  expect(canvasBox).not.toBeNull();
  const elevationBeforeDrag = (await readCanvasRecord<number>(page, "data-civil-elevations-mm"))[floorId];
  await page.mouse.move(canvasBox!.x + dragPoint.x, canvasBox!.y + dragPoint.y);
  await page.mouse.down();
  await page.mouse.move(canvasBox!.x + dragPoint.x + 24, canvasBox!.y + dragPoint.y + 12, { steps: 6 });
  await page.mouse.up();
  await expect(canvas).toHaveAttribute("data-last-scene-drag-result", "applied");
  expect((await readCanvasRecord<number>(page, "data-civil-elevations-mm"))[floorId]).toBe(elevationBeforeDrag);
  expect(errors).toEqual([]);
});
...
test("P1-D1 preferences start as Custom Workspace without an implicit Sales reset", async ({ page }) => {
  const errors = collectPageErrors(page);
  await page.setViewportSize({ width: 1440, height: 900 });
  ...
  await expect(page.getByTestId("workspace-preferences-trigger")).toContainText("Custom Workspace");
  await expect(page.getByTestId("design-system-root")).toHaveAttribute("data-av-theme", "light");
  await expect(page.getByTestId("design-system-root")).toHaveAttribute("data-av-density", "compact");
  ...
  await expect(control.popover.getByTestId("workspace-preferences-workspace-trigger"))
    .toContainText("Custom Workspace");
...
test("theme retains workspace identity and density override returns to Custom Workspace", async ({ page }) => {
  const errors = collectPageErrors(page);
  await openCleanApp(page);
  await waitForUiPreferences(page);
  const workspace = await openPreferenceBranch(page, "workspace");
...
test("workspace panel controls follow live Connection Point Snap and Inspector availability", async ({ page }) => {
  ...
  await expect(control.trigger).toContainText("Custom Workspace");
  await expect(page.getByTestId("viewport-arrange-bar").getByRole("button", { name: "Connect & Snap" })).toHaveCount(0);
  expect(await getRuntimePanel(page, "panel.connectionPointSnap")).toMatchObject({
    visible: false,
    open: false
...
test("1024 responsive presentation preserves viewport dominance and the persisted desktop Inspector state", async ({ page }) => {
  const errors = collectPageErrors(page);
  await page.setViewportSize({ width: 1440, height: 900 });
  await openCleanApp(page);
  await waitForUiPreferences(page);
  const canvas = page.getByLabel("AtrVisu 3D workspace");
  const lifecycleGeneration = await canvas.getAttribute("data-scene-lifecycle-generation");
  const persistedInspector = async () => page.evaluate(() =>
    window.__atrvisuUiPreferences?.getSnapshot().preferences.panels.find(
      (panel) => panel.panelId === "panel.rightPanelShell"
    )
  );

  await expect(page.getByTestId("right-panel")).toBeVisible();
  const wideOpenPreference = await persistedInspector();
  expect(wideOpenPreference).toMatchObject({ visible: true, collapsed: false });

  await page.setViewportSize({ width: 1024, height: 768 });
  await expect(page.getByTestId("right-panel")).toHaveCount(0);
  await expect(page.getByTestId("right-dock-collapse-toggle")).toHaveAttribute("aria-label", "Expand Inspector");
  expect(await persistedInspector()).toEqual(wideOpenPreference);
  const search = page.getByRole("button", { name: "Search commands" });
  await expect(search).toBeVisible();
  expect(await search.evaluate((element) => getComputedStyle(element).whiteSpace)).toBe("nowrap");
  await expect(search.locator(".workbench-command-search-label-compact")).toBeVisible();
  await expect(search.locator(".workbench-command-search-label-full")).toBeHidden();
  ...
  await page.getByTestId("right-dock-collapse-toggle").click();
  await expect(page.getByTestId("right-panel")).toHaveCount(0);
  const wideCollapsedPreference = await persistedInspector();
  expect(wideCollapsedPreference).toMatchObject({ visible: true, collapsed: true });
  await page.setViewportSize({ width: 1024, height: 768 });
  await page.getByTestId("right-dock-collapse-toggle").click();
  await expect(page.getByTestId("right-panel")).toBeVisible();
  expect(await persistedInspector()).toEqual(wideCollapsedPreference);
  await page.setViewportSize({ width: 1440, height: 900 });
  await expect(page.getByTestId("right-panel")).toHaveCount(0);
  await expect(canvas).toHaveAttribute("data-scene-lifecycle-generation", lifecycleGeneration ?? "");
  expect(errors).toEqual([]);
...
test("PF-3A Inspector modes and group selection avoid React update-depth feedback", async ({ page }) => {
  ...
  const primaryDock = page.getByTestId("primary-dock");
  const primaryDockToggle = page.getByTestId("primary-dock-collapse-toggle");
  await primaryDockToggle.click();
  await expect(primaryDock).toHaveAttribute("data-collapsed", "true");
  await primaryDockToggle.click();
  await expect(primaryDock).toHaveAttribute("data-collapsed", "false");
  ...
  await openPrimaryDockPanel(page, "panel.groups");
  await group.locator(".assembly-group-button").click();
  const positionsBeforeBodyDrag = await readCanvasRecord<PlanPosition>(page, "data-machine-plan-positions");
  await dragSceneMachine(page, machineIds[0], 32, 0);
  await expect.poll(() => readCanvasRecord<PlanPosition>(page, "data-machine-plan-positions"))
    .not.toEqual(positionsBeforeBodyDrag);
...
test("PF-3A persisted collapsed shells remain console-clean through body drag and hard reload", async ({ page }) => {
  const errors = collectPageErrors(page);
  await page.setViewportSize({ width: 1440, height: 900 });
  await seedUiPreferences(page, createE2EUiPreferences({
    panelOverrides: {
      "panel.rightPanelShell": { visible: true, collapsed: true },
      "panel.primaryDockShell": { visible: true, collapsed: true }
    }
  }));
  await page.goto("/?e2eDiagnostics=1");
  await expect(page.getByTestId("app-root")).toBeVisible();
  ...
  const firstAssetCard = page.getByTestId("machine-library-panel").locator(".asset-card").first();
  await firstAssetCard.getByRole("button", { name: /^Add .* to layout$/ }).click();
  await waitForMachineDiagnostics(page, 1);
  const machineId = (await getMachineIds(page))[0];
  const canvas = page.getByLabel("AtrVisu 3D workspace");
  const lifecycleGeneration = await canvas.getAttribute("data-scene-lifecycle-generation");
  const before = (await readCanvasRecord<PlanPosition>(page, "data-machine-plan-positions"))[machineId];
  await dragSceneMachine(page, machineId, 70, 40);
  await expect.poll(async () => {
  ...
  await expect(page.getByTestId("editor-host")).toHaveCount(1);
  await expect(page.locator("canvas.scene-canvas")).toHaveCount(1);
  await capturePf3aScreenshot(page, "33-console-clean-persisted-state.png");
  expect(errors).toEqual([]);
});
